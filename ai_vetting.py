...
def generate_vetting_analysis(referral_text, modality_hint=None, protocol_titles=None):
    ...
    user_prompt = (
        ...
        "- Head injury / trauma / fall / assault / RTC → ALWAYS non-contrast CT Brain (NICE CG176). NEVER contrast.\n"
        "- Stroke / TIA → non-contrast CT Brain ± CTA (CT Stroke Protocol). NOT contrast CT.\n"
        "- Headache (non-traumatic, no red flags) → non-contrast CT Brain.\n"
        "- Renal colic / ureteric stones → non-contrast CT KUB (NICE NG143).\n"
        "- ?PE → CTPA (IV contrast, arterial phase).\n"
        "- Staging / known malignancy → IV contrast (portal venous ± arterial).\n"
        "- Infection / abscess → IV contrast.\n"
        "- When selecting matched_protocol_slug, pick the NON-CONTRAST routine protocol "
        "for trauma/injury/falls — e.g. slug 'brain' not 'brain-tumours-primary'.\n\n"
