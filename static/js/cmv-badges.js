 ...
 *
 * Admin: inline badges + popup actions (full QA workbench).
 * Users: summary trust bar + collapsible verification log table at bottom.
 *
...
(function() {
    ...
    function _renderTrustBadgeAdmin(containerEl, summary) {
    ...
    // ── Trust badge (user-facing — clean summary) ──

    function _renderTrustBadgeUser(containerEl, summary, claims) {
        if (!summary || summary.total === 0) return; // No badge for unverified content

        // Count high-confidence AI agreements and expert-reviewed claims
        var aiConfirmed = 0;
        var expertReviewed = 0;
        var unresolvedDisputes = 0;
        (claims || []).forEach(function(c) {
            if (c.badge_state === 'admin_verified') {
                expertReviewed++;
            } else if (c.badge_state === 'agreed' && (c.gemini_confidence || '').toLowerCase() === 'high') {
                aiConfirmed++;
            } else if (c.badge_state === 'disputed') {
                unresolvedDisputes++;
            }
        });
        ...
        containerEl.insertAdjacentHTML('afterbegin',
            '<div class="cmv-trust-badge cmv-trust-passed mb-2"><div class="cmv-trust-header">' +
            '<i class="fas fa-shield-alt me-2"></i>' +
            '<strong>Cross-Verified</strong> ' +
            '<span class="cmv-trust-detail">' + total + ' factual claims checked — ' + parts.join(', ') + '</span>' +
            '<button class="cmv-info-btn btn btn-link btn-sm p-0 ms-2" data-bs-toggle="popover" data-bs-trigger="hover focus" data-bs-html="true" data-bs-placement="bottom" ' +
            'data-bs-content="Every piece of AI-generated content on RadInsights undergoes Cross-Model Verification (CMV), where an independent AI model checks factual claims. ' +
            'Claims flagged for review are examined and corrected by our expert panel before publication. ' +
            'You are advised to treat every piece of information as suggestive and not prescriptive.">' +
            '<i class="fas fa-info-circle"></i></button></div></div>');
    }

    // ── Verification log table (user-facing — bottom of content) ──
    ...
    function _renderVerificationTable(containerEl, claims) {
        ...
        if (tableClaims.length === 0) return;

        // Build claim-to-element mapping for click-to-highlight
        var contentEls = [];
        containerEl.querySelectorAll('li, td, p, div[data-field]').forEach(function(el) {
            if (el.tagName === 'DIV' && el.querySelector('ul, ol, table, p')) return;
            // Skip the verification table itself and trust badge
            if (el.closest('.cmv-verification-log') || el.closest('.cmv-trust-badge')) return;
            var text = (el.textContent || '').replace(/\s+/g, ' ').trim().toLowerCase();
            if (text.length > 3) contentEls.push({ el: el, text: text });
        });
        ...
        tableClaims.forEach(function(c, idx) {
            var statusHtml = '';
            var detailHtml = '';
            ...
            // Check if we can find this claim text in the content (for click-to-highlight)
            var claimText = (c.claim_text || '').replace(/\s*\|\s*/g, ' ').trim();
            var canHighlight = _findClaimElement(claimText, contentEls) !== null;

            html += '<tr class="cmv-vlog-row" data-claim-idx="' + idx + '">' +
                '<td class="small">' + _escHtml(claimText) + '</td>' +
                '<td>' + statusHtml + '</td>' +
                '<td>' + detailHtml + '</td>' +
                '<td>' + (canHighlight
                    ? '<button class="btn btn-sm btn-link p-0 cmv-highlight-btn" data-claim-text="' + _escHtml(claimText).replace(/"/g, '&quot;') + '" title="Highlight in content"><i class="fas fa-crosshairs text-muted"></i></button>'
                    : '') +
                '</td></tr>';
        });
        ...
        // Bind click-to-highlight handlers
        containerEl.querySelectorAll('.cmv-highlight-btn').forEach(function(btn) {
            btn.addEventListener('click', function(e) {
                e.preventDefault();
                var text = this.dataset.claimText;
                _highlightClaimInContent(text, contentEls, containerEl);
            });
        });
    }

    function _findClaimElement(claimText, contentEls) {
        var searchText = claimText.replace(/\s+/g, ' ').toLowerCase();
        var words = searchText.split(/\s+/);
        var searches = [searchText];
        if (words.length >= 5) searches.push(words.slice(0, 5).join(' '));
        if (words.length >= 3) searches.push(words.slice(0, 3).join(' '));

        for (var s = 0; s < searches.length; s++) {
            for (var i = 0; i < contentEls.length; i++) {
                if (contentEls[i].text.indexOf(searches[s]) !== -1) return contentEls[i];
            }
        }
        return null;
    }

    function _highlightClaimInContent(claimText, contentEls, containerEl) {
        // Remove any existing highlights
        containerEl.querySelectorAll('.cmv-text-highlight').forEach(function(el) {
            el.classList.remove('cmv-text-highlight');
        });

        var match = _findClaimElement(claimText, contentEls);
        if (match) {
            match.el.scrollIntoView({ behavior: 'smooth', block: 'center' });
            match.el.classList.add('cmv-text-highlight');
            setTimeout(function() {
                match.el.classList.remove('cmv-text-highlight');
            }, 3000);
        }
    }
    ...
    window.CmvBadges = {
         ...
         * Admin: inline badges + popup actions.
         * Users: summary trust bar + collapsible verification log table.
         *
        ...
        load: function(containerEl, contentType, contentId) {
            ...
            var isAdmin = _checkAdmin();

            fetch('/api/peer-review/claims?content_type=' + encodeURIComponent(contentType) + '&content_id=' + encodeURIComponent(contentId))
                .then(function(r) { return r.json(); })
                .then(function(data) {
                    var hasClaims = data.claims && data.claims.length > 0;
                    var summary = hasClaims ? data.summary : null;

                    if (isAdmin) {
                        // Admin: full inline badges + detailed trust bar
                        _renderTrustBadgeAdmin(containerEl, summary);
                        if (hasClaims) {
                            _injectBadges(containerEl, data.claims);
                            _renderReferences(containerEl, data.claims);
                        }
                    } else {
                        // User: clean summary trust bar + verification log table
                        if (hasClaims) {
                            _renderTrustBadgeUser(containerEl, summary, data.claims);
                            _renderVerificationTable(containerEl, data.claims);
                            _renderReferences(containerEl, data.claims);
                        }
                    }
                    ...
                    if (isAdmin && hasClaims && !containerEl.dataset.cmvClickReady) {
                    ...
                    // Auto-scroll to claim if ?claim=<id> is in URL (admin only — users don't have inline badges)
                    if (isAdmin) _scrollToClaimFromUrl();
                });
        },
