"""
AI On-Call Helper Module

Provides curated, source-traceable clinical protocol answers for on-call radiologists.
...
def generate_oncall_response(query_text, user_id, provider='claude', model=None):
    ...
    if not api_key:
        raise OnCallHelperError("RadInsight Intelligence API key not configured.")
...
actionable answer formatted for a radiologist on call. Include source citations."""
    ...
    try:
        ...
        raise OnCallHelperError("RadInsight Intelligence request timed out. Please try again.")
    except requests.exceptions.RequestException as exc:
        raise OnCallHelperError(f"Failed to connect to RadInsight Intelligence: {exc}")

    if response.status_code >= 300:
        detail = response.text[:500]
        raise OnCallHelperError(f"RadInsight Intelligence error (HTTP {response.status_code}): {detail}")

    result = response.json()
    content = result.get("content", [])
    if not content:
        raise OnCallHelperError("Empty response from RadInsight Intelligence.")

    text = content[0].get("text", "")
    if not text:
        raise OnCallHelperError("No text in RadInsight Intelligence response.")
...
Category is one of: criteria, scoring, grading, staging, emergency, pathway, imaging_features,
management_support, contrast, dose, safety, anatomy.
...
def generate_protocol_content(title, category, source_citation='', additional_context=''):
    ...
    try:
        ...
        raise OnCallHelperError("Protocol generation timed out. Please try again.")
    except requests.exceptions.RequestException as exc:
        raise OnCallHelperError(f"Failed to connect to RadInsight Intelligence: {exc}")
    ...
    try:
        ...
        if match:
            try:
                ...
                raise OnCallHelperError("Failed to parse protocol generation response as JSON.")
        else:
            raise OnCallHelperError("Protocol generation response was not valid JSON.")
