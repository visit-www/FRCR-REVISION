...
- **Image Stack tab** – New tab in view_case with plan selector and Prev/Next slice navigation (basic img-based viewer)
...
### 🔲 Remaining (per plan)
1. **Cornerstone3D viewer** – Replace basic `<img>` viewer with Cornerstone3D for proper DICOM-like stack scrolling
2. **Image proxy (optional)** – OneDrive `@microsoft.graph.downloadUrl` may have CORS/expiry; add backend proxy if needed
3. **OneDrive browse** – Allow browsing signed-in user's OneDrive instead of only pasting share links (discussed, not implemented)
...
| File | Purpose |
|------|---------|
| `case_dicom_viewer/routes.py` | OAuth, status, folder/parse, case stack CRUD |
...
| `templates/view_case.html` | Image Stack tab, `loadImageStack()`, slice nav |
...
1. Test full flow: Connect OneDrive → paste share link → parse → save → view in Image Stack tab.
2. If images don't load (CORS), add proxy: `GET /case-dicom-viewer/api/image?url=...` that fetches from OneDrive and streams.
3. Integrate Cornerstone3D for stack scrolling.
4. Optionally add OneDrive folder browser UI.
